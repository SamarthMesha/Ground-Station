...
// Enhanced with hamburger menu navigation system
...
const domElements = {
    ...
    gyrationRate: document.getElementById('gyration-rate'),
    rocketSpin: document.getElementById('rocket-spin'),
    thrustLevel: document.getElementById('thrust-level'),
    
    // Flight visualization
    flightMode: document.getElementById('flight-mode'),
    vizVelocity: document.getElementById('viz-velocity'),
    vizDynPressure: document.getElementById('viz-dyn-pressure'),
    vizAcceleration: document.getElementById('viz-acceleration'),
    
    // GPS elements
    distanceToRocket: document.getElementById('distance-to-rocket'),
    bearingToRocket: document.getElementById('bearing-to-rocket'),
    ...
    gpsVelocity: document.getElementById('gps-velocity'),
    ...
    apogeeVelocity: document.getElementById('apogee-velocity'),
    ...
    showShortestPath: document.getElementById('show-shortest-path'),
    toggleTerrain: document.getElementById('toggle-terrain'),
    toggleSatellite: document.getElementById('toggle-satellite'),
    toggleRocketRotate: document.getElementById('toggle-rocket-rotate'),
    resetRocketView: document.getElementById('reset-rocket-view'),
    toggleExhaust: document.getElementById('toggle-exhaust'),
...
function initializeApp() {
    ...
    addLogEntry('SYSTEM', 'Mission Control initialized with hamburger menu navigation');
...
function openPanel(panelId) {
    ...
    // Hide control buttons row
    const controlButtons = document.querySelector('.control-buttons-row');
    if (controlButtons) {
...
function animate3DRocket() {
    ...
    // Rotate rocket if enabled
    if (AppState.isRocketRotating && AppState.rocket) {
        AppState.rocket.rotation.y += 0.01;
    }
    
    // Update rocket stats
    if (AppState.rocket) {
        const spinRate = AppState.isRocketRotating ? 0.6 : 0;
        if (domElements.rocketSpin) domElements.rocketSpin.textContent = `${spinRate.toFixed(1)} RPM`;
        if (domElements.gyrationRate) domElements.gyrationRate.textContent = `${(Math.abs(AppState.rocket.rotation.y) * 57.3).toFixed(1)}°/s`;
        if (domElements.rocketOrientation) domElements.rocketOrientation.textContent = AppState.isRocketRotating ? 'Rotating' : 'Stable';
    }
...
function updateTelemetryDisplay(data) {
    ...
    if (data.accelX !== undefined && data.accelY !== undefined && data.accelZ !== undefined) {
        ...
        if (domElements.vizAcceleration) domElements.vizAcceleration.textContent = `${total.toFixed(2)} G`;
    } else {
        if (domElements.totalAccel) domElements.totalAccel.textContent = '-- G';
        if (domElements.vizAcceleration) domElements.vizAcceleration.textContent = '-- G';
    }
    ...
    if (data.ignition !== undefined && domElements.ignitionStatus) {
        ...
        // Update thrust level
        if (domElements.thrustLevel) {
            domElements.thrustLevel.textContent = isIgnited ? '85%' : '0%';
        }
    }
    ...
    updateElement(data.velocity, domElements.vizVelocity, `${data.velocity?.toFixed(1) || '--'} m/s`);
    updateElement(data.velocity, domElements.gpsVelocity, `${data.velocity?.toFixed(1) || '--'} m/s`);
    updateElement(data.dynamicPressure, domElements.vizDynPressure, `${((data.dynamicPressure || 0) / 1000).toFixed(2) || '--'} kPa`);
    
    // Flight mode
    if (data.altitude !== undefined && domElements.flightMode) {
        if (data.altitude < 100) {
            domElements.flightMode.textContent = 'LAUNCH';
        } else if (data.altitude < 1000) {
            domElements.flightMode.textContent = 'BOOST PHASE';
        } else if (data.altitude < 10000) {
            domElements.flightMode.textContent = 'ASCENT';
        } else if (data.verticalVelocity < 0) {
            domElements.flightMode.textContent = 'DESCENT';
        } else {
            domElements.flightMode.textContent = 'COAST';
        }
    } else if (domElements.flightMode) {
        domElements.flightMode.textContent = 'STANDBY';
    }
...
function updatePanelSpecificData(data) {
    ...
    if (AppState.activePanel === 'trajectory') {
        ...
        updateElement(data.pitch, domElements.pitchAngle, `${data.pitch?.toFixed(1) || '--'}°`);
        updateElement(data.angularRate, domElements.rollRate, `${data.angularRate?.toFixed(1) || '--'}°/s`);
    }
    ...
    if (AppState.activePanel === 'tracking') {
        ...
        updateElement(data.velocity, domElements.gpsVelocity, `${data.velocity?.toFixed(1) || '--'} m/s`);
        updateElement(data.verticalVelocity, domElements.verticalSpeed, `${data.verticalVelocity?.toFixed(1) || '--'} m/s`);
        updateElement(data.gpsSats, domElements.gpsSatellites, data.gpsSats || '--');
        
        // Update statistics
        if (data.velocity !== undefined) {
            if (data.velocity > parseFloat(domElements.maxVelocity?.textContent || 0)) {
                if (domElements.maxVelocity) {
                    domElements.maxVelocity.textContent = `${data.velocity.toFixed(1)} m/s`;
                }
            }
        }
        
        // Update average temperature
        if (data.temperature && domElements.avgTemperature) {
            const avgTemp = calculateAverageTemperature(data.temperature);
            domElements.avgTemperature.textContent = `${avgTemp.toFixed(1)} °C`;
        }
    }
}
...
function initializeEventListeners() {
    ...
    if (domElements.resetAllButton) {
        domElements.resetAllButton.addEventListener('click', resetMission);
    }
    if (domElements.resetButton) {
        domElements.resetButton.addEventListener('click', resetMission);
    }
    if (domElements.exportButton) {
        domElements.exportButton.addEventListener('click', exportDataCSV);
    }
    ...
    // 3D Rocket controls
    if (domElements.toggleRocketRotate) {
        domElements.toggleRocketRotate.addEventListener('click', () => {
            AppState.isRocketRotating = !AppState.isRocketRotating;
            const btn = domElements.toggleRocketRotate;
            if (AppState.isRocketRotating) {
                btn.innerHTML = '<i class="fas fa-pause"></i><span>Stop Rotate</span>';
    ...
    if (domElements.resetRocketView) {
        domElements.resetRocketView.addEventListener('click', () => {
            if (AppState.camera) {
                AppState.camera.position.set(0, 10, 25);
                AppState.camera.lookAt(0, 0, 0);
            }
            ...
            if (domElements.toggleRocketRotate) {
                domElements.toggleRocketRotate.innerHTML = '<i class="fas fa-sync-alt"></i><span>Auto Rotate</span>';
                domElements.toggleRocketRotate.classList.remove('active');
            }
        });
    }
    
    // Map controls
    if (domElements.showShortestPath) {
        domElements.showShortestPath.addEventListener('click', toggleShortestPath);
    }
    if (domElements.toggleTerrain) {
        domElements.toggleTerrain.addEventListener('click', () => toggleMapType('terrain'));
    }
    ...
    document.querySelectorAll('.chart-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            handleChartControl(action, this.closest('.chart-container'));
        });
    });
}

function handleChartControl(action, chartContainer) {
...
function exportChartData(chartId) {
    ...
    switch(chartId) {
        case 'accelerationChart':
            data = AppState.chartData.time.map((t, i) => ({
                time: t,
                accelX: AppState.chartData.accelX[i] || 0,
                accelY: AppState.chartData.accelY[i] || 0,
                accelZ: AppState.chartData.accelZ[i] || 0,
                totalAccel: AppState.chartData.totalAccel[i] || 0
            }));
        ...
        case 'altitudeChart':
            data = AppState.chartData.time.map((t, i) => ({
                time: t,
                altitude: AppState.chartData.altitude[i] || 0
            }));
            filename = 'altitude_data.csv';
            break;
        case 'pressureChart':
            data = AppState.chartData.altitude.map((alt, i) => ({
                altitude: alt,
                pressure: AppState.chartData.pressure[i] || 0
            }));
            filename = 'pressure_data.csv';
            break;
        case 'trajectoryChart':
            data = trajectoryChart.data.datasets[0].data;
    ...
    addLogEntry('SYSTEM', `Exported ${chartId} data to CSV`);
}
...
// ===== DATA INPUT FUNCTION =====
// This function can be called from external systems to input real telemetry data
...
function calculateAverageTemperature(currentTemp) {
    if (!currentTemp) return '--';
    
    // Simple moving average calculation
    const recentTemps = AppState.telemetryData.slice(-10).map(d => d.temperature).filter(t => t !== undefined);
    if (recentTemps.length === 0) return currentTemp;
    
    const sum = recentTemps.reduce((a, b) => a + b, 0);
...
function updateGPSData(data) {
    if (data.latitude !== undefined && data.longitude !== undefined && AppState.googleMap) {
        ...
        updateDistanceCalculations(position, data.altitude || 0);
...
function updateDistanceCalculations(rocketPosition, altitude) {
    const groundDist = calculateGroundDistance(
        AppState.groundStationPosition.lat,
        AppState.groundStationPosition.lng,
        rocketPosition.lat,
        rocketPosition.lng
    );
    
    const directDist = Math.sqrt(groundDist * groundDist + altitude * altitude);
    ...
    if (domElements.distanceToRocket) {
        domElements.distanceToRocket.textContent = `${(directDist / 1000).toFixed(1)} km`;
    }
    if (domElements.bearingToRocket) {
        domElements.bearingToRocket.textContent = `${bearing.toFixed(0)}°`;
    }
    if (domElements.distanceTraveled) {
        domElements.distanceTraveled.textContent = `${(directDist / 1000).toFixed(1)} km`;
    }
}

function calculateGroundDistance(lat1, lon1, lat2, lon2) {
    const R = 6371000;
...
function toggleShortestPath() {
    if (AppState.googleMap && AppState.launchSiteMarker && AppState.flightMarker) {
        // Clear existing path
        if (AppState.flightPath) {
            AppState.flightPath.setMap(null);
        }
        
        // Create straight line path
        const launchPos = AppState.launchSiteMarker.getPosition();
        const rocketPos = AppState.flightMarker.getPosition();
        
        AppState.flightPath = new google.maps.Polyline({
            path: [launchPos, rocketPos],
            geodesic: true,
            strokeColor: '#10b981',
            strokeOpacity: 1.0,
            strokeWeight: 3,
        ...
        addLogEntry('SYSTEM', 'Shortest path display enabled');
    }
}
...
function closeAllPanels() {
    ...
    const elementsToShow = [
        ...
        document.querySelector('.control-buttons-row')
    ];
    ...
    if (document.querySelector('.control-buttons-row')) {
        document.querySelector('.control-buttons-row').style.display = 'flex';
    }
}
...
function resetAllDisplays() {
    ...
    const elementsToReset = [
        ...
        { element: domElements.gyrationRate, value: '0°/s' },
        { element: domElements.rocketSpin, value: '0 RPM' },
        { element: domElements.thrustLevel, value: '0%' },
        { element: domElements.flightMode, value: 'STANDBY' },
        { element: domElements.vizVelocity, value: `${resetValue} m/s` },
        { element: domElements.vizDynPressure, value: `${resetValue} kPa` },
        { element: domElements.vizAcceleration, value: `${resetValue} G` },
        ...
        { element: domElements.gpsVelocity, value: `${resetValue} m/s` },
        { element: domElements.verticalSpeed, value: `${resetValue} m/s` },
        { element: domElements.gpsSatellites, value: resetValue },
        { element: domElements.distanceToRocket, value: `${resetValue} km` },
        { element: domElements.bearingToRocket, value: `${resetValue}°` },
        ...
        { element: domElements.apogeeVelocity, value: `${resetValue} m/s` },
        { element: domElements.flightDuration, value: '--:--' },
        { element: domElements.distanceTraveled, value: `${resetValue} km` },
        { element: domElements.flightTime, value: '--:--' },
        { element: domElements.maxVelocity, value: `${resetValue} m/s` },
        { element: domElements.avgTemperature, value: `${resetValue} °C` },
        { element: domElements.pitchAngle, value: `${resetValue}°` },
        { element: domElements.rollRate, value: `${resetValue}°/s` },
        { element: domElements.loadFactor, value: `${resetValue} G` },
        { element: domElements.aoa, value: `${resetValue}°` }
    ];
